# 🌊 CalmWave - Painel Administrativo & API (ADM_CalmWave)

Bem-vindo ao módulo de **Servidor e Dashboard Web** do ecossistema CalmWave. 

> **Nota:** Este é o repositório do painel administrativo e do back-end. Para o aplicativo Android principal de captação e IA offline, acesse a pasta [Mobile_CalmWave](../Mobile_CalmWave/README.md).

Este sistema foca no recebimento de áudios, transcrição automatizada textual e análise de dados das gravações vindas do mobile. Ele possui um robusto banco de dados (Supabase/PostgreSQL) e um painel analítico em tempo real.

O módulo ADM está dividido internamente em duas partes principais:

1. **Back-End API** (`back-end/`): Escrito em Python utilizando Flask, Supabase (banco de dados PostgreSQL), JWT para autenticação e dependências de áudio (PyTorch com manipulação de bytes de áudio para processamento na nuvem e SpeechRecognition para transcrição).
...
Para detalhes estruturais completos sobre tabelas, eventos e estatísticas, veja a [Documentação de Banco de Dados e Analytics](./doc.md).
